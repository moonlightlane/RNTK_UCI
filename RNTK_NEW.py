...
class RNTK():
    ...
    def get_diag_indices(self):
        ...
        dim_1_i = self.dim_1
        dim_2_i = 0
        dim_num = self.dim_1 + self.dim_2 + 1
    ...
    def make_inputs(self, dim1idx, dim2idx):
    ...
    def create_func_for_diag(self, dim1idx, dim2idx, function = False):
        diag, dim1ph, dim2ph, dim1idxph, dim2idxph, nph = self.make_inputs(dim1idx, dim2idx)
    ...
    def diag_func_wrapper(self, dim_1_idx, dim_2_idx, fbool = False):
        f = self.create_func_for_diag(dim_1_idx, dim_2_idx, function = fbool)
        if fbool:
    ...
    def arrays_to_diag(self, array_of_diags):
        ...
        for i in range(0, self.dim_1 + 1): #these are rows
            ...
            for j in range(0, self.dim_2 + 1): #these are columns
                list_index = min(self.dim_2-i, j) #could be dim 1
                which_list = j + i
                new_list_idx = list_index + self.index_func(which_list, self.dim_1, self.dim_2)
